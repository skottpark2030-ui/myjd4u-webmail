...
// 메일함 경로 찾기 (special-use \Sent / \Trash 우선)
async function resolveBox(client, which) {
  ...
  if (which === 'TRASH') {
    try {
      const list = await client.list();
      const t = list.find(m => m.specialUse === '\\Trash')
        || list.find(m => /^(trash|deleted|휴지통)/i.test(m.path));
      return t ? t.path : 'Trash';
    } catch (_) { return 'Trash'; }
  }
  return 'INBOX';
}
...
// 삭제 → 휴지통으로 이동 (받은/보낸편지함에서). 휴지통에서 호출하면 영구 삭제.
app.post('/api/delete/:uid', requireAuth, async (req, res) => {
  const which = (req.query.box || 'INBOX').toUpperCase();
  const client = newImap(req.session.creds);
  try {
    await client.connect();
    ...
    try {
      if (which === 'TRASH') {
        await client.messageDelete(req.params.uid, { uid: true }); // 영구 삭제
        res.json({ ok: true, purged: true });
      } else {
        const trash = await resolveBox(client, 'TRASH');
        await client.messageMove(req.params.uid, trash, { uid: true }); // 휴지통으로
        res.json({ ok: true, movedTo: trash });
      }
    } finally { lock.release(); }
    await client.logout();
  } catch (e) { res.status(500).json({ error: '삭제하지 못했습니다: ' + e.message }); }
});

// 휴지통 → 받은편지함으로 복원
app.post('/api/restore/:uid', requireAuth, async (req, res) => {
  const client = newImap(req.session.creds);
  try {
    await client.connect();
    const trash = await resolveBox(client, 'TRASH');
    ...
    try {
      await client.messageMove(req.params.uid, 'INBOX', { uid: true });
      res.json({ ok: true });
    } finally { lock.release(); }
    await client.logout();
  } catch (e) { res.status(500).json({ error: '복원하지 못했습니다: ' + e.message }); }
});
...
app.post('/api/empty-trash', requireAuth, async (req, res) => {
  ...
  try {
    ...
    await client.logout();
  } catch (e) { res.status(500).json({ error: '휴지통을 비우지 못했습니다: ' + e.message }); }
});
